...
class CASE27_Integrations_Bookmark {
	...
	public function __construct()
	{
		...
		add_action('template_redirect', function() {
			// Handle actions.
			if ( isset( $_GET['action'] ) && $_GET['action'] && is_wc_endpoint_url( '27-bookmarks' ) ) {
		...
		switch ($action) {
			case 'remove_bookmark':
				if ($listingID && $this->remove_bookmark( $listingID )) {
					...
					wp_safe_redirect( wc_get_endpoint_url( '27-bookmarks' ) );
					exit;
				}
			break;
		}
...
CASE27_Integrations_Bookmark::instance();
