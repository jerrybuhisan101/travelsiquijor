...
class PromotedListings {
	...
	public function __construct()
	{
		...
		add_action('template_redirect', function() {
			// Handle actions.
			if ( isset( $_GET['action'] ) && $_GET['action'] && is_wc_endpoint_url( '27-promoted-listings' ) ) {
		...
		add_action('job_manager_job_dashboard_do_action_case27_promote', function() {
			if ( $listingID = absint( $_REQUEST['job_id'] ) ) {
				wp_safe_redirect( add_query_arg( 'listing_id', $listingID, wc_get_endpoint_url( '27-promoted-listings' ) ) );
				exit;
			}
		});
		...
		switch ($action) {
			case 'promote_listing':
				if ($listingID && $keyID && $this->promote_listing($listingID, $keyID)) {
					...
					wp_safe_redirect( wc_get_endpoint_url( '27-promoted-listings' ) );
					exit;
				}
			break;
	...
	public function get_listing( $listingID )
	{
		...
		if ( (int) $listing->post_author && ((int) $listing->post_author !== (int) get_current_user_id()) ) {
