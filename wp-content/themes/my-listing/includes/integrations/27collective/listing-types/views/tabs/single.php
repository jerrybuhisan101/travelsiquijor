					...
					<div class="form-group">
						...
						<div class="select-wrapper">
							<select v-model="state.single.active_button.action">
								<option value="custom-field">Show a field's value</option>
								<option value="share">Share Listing</option>
								<option value="bookmark">Bookmark Listing</option>
								<option value="add-review">Add Review</option>
								<option value="book">Book</option>
								<option value="display-rating">Display Rating</option>
							</select>
						</div>
					</div>
		...
		<div class="form-group edit-page" v-if="state.single.active_menu_item">
			<div class="cover-button-wrapper">
				<div class="cover-button-options">
					...
					<div class="form-group" v-if="state.single.active_menu_item.page == 'comments'">
						<br><br>
						<label><input type="checkbox" v-model="state.single.active_menu_item.allow_rating"> Allow Rating? (Reviews) </label>
					</div>
				...
				<draggable v-model="state.single.active_menu_item.layout" :options="{group: 'layout-blocks', animation: 100, draggable: '.field', filter: '.no-drag', handle: 'h5'}" @start="drag=true" @end="drag=false" class="fields-draggable" :class="drag ? 'active' : ''">
					<div v-for="block in state.single.active_menu_item.layout" class="field">
						...
						<div class="options edit">
							...
							<div v-if="block.options" v-for="option in block.options" class="form-group" :class="option.type">
												...
												<div class="form-group" v-if="option.fields.indexOf('show_field') > -1">
													...
													<div class="select-wrapper">
														<select v-model="row.show_field">
															...
															<option v-for="field in fieldsByType(['text', 'wp-editor', 'checkbox', 'select', 'multiselect', 'textarea', 'date', 'time', 'datetime', 'work-hours', 'email', 'url', 'number'])" :value="field.slug">{{ field.label }}</option>
