								...
								<div class="form-group">
									...
									<div class="select-wrapper">
										<select v-model="button.show_field">
											...
											<option v-for="field in fieldsByType(['text', 'wp-editor', 'checkbox', 'select', 'multiselect', 'textarea', 'date', 'time', 'datetime', 'work-hours', 'email', 'url', 'number', 'location'])" :value="field.slug">{{ field.label }}</option>
								...
								<div class="form-group">
									...
									<div class="select-wrapper">
										<select v-model="field.show_field">
											<option value="" disabled="disabled">Select a field...</option>
											<option v-for="field in fieldsByType(['text', 'wp-editor', 'checkbox', 'select', 'multiselect', 'textarea', 'date', 'time', 'datetime', 'email', 'url', 'number', 'location'])" :value="field.slug">{{ field.label }}</option>
										</select>
									</div>
								</div>
							</div>
				...
				<div class="card fields-wrapper">
					...
					<draggable v-model="result.footer.sections" :options="{group: 'result-footer.sections', animation: 100, handle: 'h5'}" @start="drag=true" @end="drag=false" class="fields-draggable" :class="drag ? 'active' : ''">
						<div v-for="section in result.footer.sections" class="footer-section field">
							...
							<div class="edit">
								<div class="form-group full-width" v-if="typeof section.label !== 'undefined' && section.type == 'host'">
									<label>Label</label>
								...
								<div v-if="typeof section.details !== 'undefined' && section.type == 'details'">
						...
						<div class="form-group full-width add-new add-new-block footer-add-new-section">
							...
							<div class="select-wrapper">
								<select id="result-add-section">
									...
									<option value="host">Related Listing (Host)</option>
