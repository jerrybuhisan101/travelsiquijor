...
class CASE27_WP_Job_Manager_Integration {
	...
	public function validate_fields($isValid, $fields, $values)
	{
		...
		foreach ( $fields['job'] as $key => $field ) {
			...
			if ( $field['type'] == 'file' && $field['slug'] == 'job_gallery' && isset( $values[$field['slug']] ) && $values[$field['slug']] && count( $values[$field['slug']] ) < 3 ) {
				return new WP_Error( 'validation-error', sprintf( __( 'You need to upload at least 3 files in %s field.', 'my-listing' ), $field['label'] ) );
			}
		}
