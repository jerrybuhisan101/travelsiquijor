...
class CASE27_WP_Job_Manager_Queries extends CASE27_Ajax {
	...
	public function __construct()
	{
		...
		$this->register_action('get_listings_by_category');
	...
	public function get_listings()
	{
		...
		foreach ($search_facets as $facet) {
			...
			if ($facet['type'] == 'proximity' && isset($form_data['proximity']) && isset($form_data['search_location_lat']) && isset($form_data['search_location_lng'])) {
				...
				$active = isset($form_data['proximity_active']) && $form_data['proximity_active'] == 'true';
				$units = isset($form_data['proximity_units']) && $form_data['proximity_units'] == 'mi' ? 'mi' : 'km';

				if ($active && $lat && $lng && $proximity && $location) {
			...
			if (($facet['type'] == 'dropdown' || $facet['type'] == 'checkboxes') && isset($form_data[$facet['show_field']]) && $form_data[$facet['show_field']]) {
				...
				$taxonomy_names = [
					'job_category' => 'job_listing_category',
					'job_tags' => 'case27_job_listing_tags'
				];

				// Tax query.
				if (in_array($facet['show_field'], array_keys($taxonomy_names))) {
					$args['tax_query'][] = [
						'taxonomy' => $taxonomy_names[$facet['show_field']],
						'field' => 'slug',
	...
	public function get_listings_by_category()
	{
		check_ajax_referer( 'c27_ajax_nonce', 'security' );

		if (!isset($_REQUEST['form_data']) || !is_array($_REQUEST['form_data'])) return;
		if (!isset($_REQUEST['category']) || !$_REQUEST['category']) return;
