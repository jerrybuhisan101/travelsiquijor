...
class CASE27_Filters {
	...
	protected $filters = [
		'query_vars',
		'acf_save_json' => 'acf/settings/save_json',
		'woocommerce_locate_template',
        'job_manager_geolocation_api_key',
        'get_the_archive_title',
        [
            'filter' => 'term_link',
            'callback' => 'listing_category_link',
            'priority' => 10,
            'accepted_args' => 3,
        ],
        'loop_shop_columns',
        'woocommerce_output_related_products_args',
        'acf_settings_show_admin' => 'acf/settings/show_admin',
    ...
    public function filter_acf_save_json($path)
    {
    	return get_stylesheet_directory() . '/includes/integrations/advanced-custom-fields/acf-json';
    }
    ...
    public function action_tgmpa_register()
    {
        ...
        $plugins = [
            ...
            [
                'name' => __( 'Advanced Custom Fields', 'my-listing' ),
            ...
            [
                'name' => __( 'WP Job Manager - WC Paid Listings', 'my-listing' ),
                'slug' => 'wp-job-manager-wc-paid-listings',
                'source' => 'wp-job-manager-wc-paid-listings.zip',
                'version' => '2.7.3',
                'required' => false,
                'force_activation' => false,
            ],
            [
                'name' => __( 'WC Vendors', 'my-listing' ),
                'slug' => 'wc-vendors',
                'required' => false,
                'force_activation' => false,
            ],
    ...
    public function filter_acf_settings_show_admin()
    {
    ...
    public function filter_listing_category_link($termlink, $term, $taxonomy)
    {
        if ($taxonomy == 'job_listing_category' && ($explore_page = c27()->get_setting('general_explore_listings_page', false))) {
    ...
    public function filter_case27_pagetop_args($pageTop)
    {
        ...
        if (is_singular('job_listing') || is_page_template('page-templates/content-featured-image.php') || (is_singular('post') && has_post_thumbnail()) || $is_buddypress_profile ) {
            $pageTop['header']['show'] = true;
            $pageTop['title-bar']['show'] = false;
