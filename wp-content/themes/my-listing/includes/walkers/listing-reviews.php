		...
		<li <?php comment_class( $parent_class . $other_classes ); ?> id="comment-<?php comment_ID() ?>">
			<div class="comment-container">
				<div class="comment-head">
					...
					<span class="comment-date"><?php comment_date() ?> at <?php comment_time() ?> <?php edit_comment_link( esc_html__('&middot; Edit', 'my-listing') ); ?></span>

					<?php if (!$is_reply && ($review_rating = CASE27_Integrations_Review::get_rating(get_comment_ID())) && $GLOBALS['case27_reviews_allow_rating']): ?>
				...
				<div class="reply comment-info">
					...
					comment_reply_link( array_merge( $args, array(
						...
						'reply_text' => '<i class="material-icons sm-icon">chat_bubble_outline</i>Reply',
						))); ?>
				</div>
			</div>
