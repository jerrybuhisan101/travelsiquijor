...
class CASE27_Helpers {
	...
	public function get_open_status_from_work_hours( $hours )
	{
		// Save the global site timezone.
		$site_timezone = date_default_timezone_get();

		// Temporarily change the timezone to match that of the listing.
		if ( ! empty( $hours['timezone'] ) ) {
			date_default_timezone_set( $hours['timezone'] );
		}

		$day = isset($hours[date('l')]) ? $hours[date('l')] : false;
		$start = ( $day && ! empty( $day['from'] ) ) ? DateTime::createFromFormat('H:i', $day['from']) : false;
		$end = ( $day && ! empty( $day['to'] ) ) ? DateTime::createFromFormat('H:i', $day['to']) : false;
		$now = $day ? DateTime::createFromFormat('H:i', date('H:i')) : false;

		// Revert timezone to the global site timezone.
		date_default_timezone_set( $site_timezone );

		if (!$start || !$end || $now < $start) {
			// Today the business is not yet open,
			// so we can check if $now is withing yesterday's schedule,
			// in case the $end date for yesterday was past midnight.
			$day = isset($hours[date('l', strtotime('-1 day'))]) ? $hours[date('l', strtotime('-1 day'))] : false;
			$start = $day ? DateTime::createFromFormat('H:i', $day['from']) : false;
			$end = $day ? DateTime::createFromFormat('H:i', $day['to']) : false;

			return $start && $end && ($end <= $start) && ($now < $end);
		}
	...
	public function get_open_message_from_work_hours( $hours )
	{
		$isOpen = $this->get_open_status_from_work_hours( $hours );
		$data = [
			'message' => $isOpen ? __( 'Open', 'my-listing' ) : __( 'Closed', 'my-listing' ),
		...
		if ( $isOpen && $now && $start && $end && ( $start->getTimestamp() !== $end->getTimestamp() ) && ( $end->getTimestamp() >= $now->getTimestamp() ) ) {
			// Time until place is closed, in minutes.
			$timeUntilClosed = ( $end->getTimestamp() - $now->getTimestamp() ) / 60;

			if ( $timeUntilClosed <= 5 ) {
				$data['message'] = __( 'Closes in a few minutes', 'my-listing' );
				$data['status'] = 'closing';
			} elseif ( $timeUntilClosed <= 30 ) {
				$data['message'] = sprintf( __( 'Closes in %d minutes', 'my-listing' ), ( round( $timeUntilClosed / 5 ) * 5 ) );
				$data['status'] = 'closing';
			}
		}

		if ( ! $isOpen && $now && $start && ( $start->getTimestamp() >= $now->getTimestamp() ) ) {
			// Time until place is opened, in minutes.
			$timeUntilOpened = ( $start->getTimestamp() - $now->getTimestamp() ) / 60;

			if ( $timeUntilOpened <= 5 ) {
				$data['message'] = __( 'Opens in a few minutes', 'my-listing' );
				$data['status'] = 'opening';
			} elseif ( $timeUntilOpened <= 30 ) {
				$data['message'] = sprintf( __( 'Opens in %d minutes', 'my-listing' ), ( round( $timeUntilOpened / 5 ) * 5 ) );
				$data['status'] = 'opening';
			}
		}
	...
	{
		if (!is_plugin_active('advanced-custom-fields-pro/acf.php')) return false;
		if (!is_plugin_active('contact-form-7/wp-contact-form-7.php')) return false;
		if (!is_plugin_active('elementor/elementor.php')) return false;
		if (!is_plugin_active('woocommerce/woocommerce.php')) return false;
		if (!is_plugin_active('wordpress-importer/wordpress-importer.php')) return false;
		if (!is_plugin_active('wp-job-manager/wp-job-manager.php')) return false;
		if (!is_plugin_active('wp-job-manager-wc-paid-listings/wp-job-manager-wc-paid-listings.php')) return false;
