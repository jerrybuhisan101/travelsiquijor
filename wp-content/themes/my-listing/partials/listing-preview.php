<?php
    ...
    $first_category = [
        'icon' => $categories ? (get_field('icon', 'job_listing_category_' . $categories[0]->term_id) ? : c27()->defaults()['category']['icon']) : '',
        'color' => $categories ? (get_field('color', 'job_listing_category_' . $categories[0]->term_id) ? : c27()->defaults()['category']['color']) : '',
        'text_color' => $categories ? (get_field('text_color', 'job_listing_category_' . $categories[0]->term_id) ? : c27()->defaults()['category']['text_color']) : '',
    ];
    ...
    $listing->_c27_marker_data = [
        ...
        'category_icon' => $first_category['icon'],
        'category_color' => $first_category['color'],
        'category_text_color' => $first_category['text_color'],
        'category_text_color' => $first_category['text_color'],
    ];
...
<div <?php job_listing_class( esc_attr( " lf-item-container listing-preview {$classes[$options['template']]} " . ($detailsCount > 2 ? ' lf-small-height ' : '') ), $listing->ID); ?>
     ...
     data-category-icon="<?php echo esc_attr( $first_category['icon'] ) ?>"
     data-category-color="<?php echo esc_attr( $first_category['color'] ) ?>"
     data-category-text-color="<?php echo esc_attr( $first_category['text_color'] ) ?>"
                ...
                <div class="lf-head">
                    ...
                    <?php foreach ($options['buttons'] as $button): ?>
                        ...
                        <?php elseif ($button['show_field'] == 'work_hours' && isset($listing_meta["_work_hours"]) && ($hours = unserialize($listing_meta["_work_hours"][0]))):
                            $open_now = c27()->get_open_status_from_work_hours($hours); ?>
                ...
                <div class="listing-details c27-footer-section">
                    <ul class="c27-listing-preview-category-list">
                        <?php if (count($categories)):
                            ...
                            ?>
                            <li>
                                <a href="<?php echo esc_url( get_term_link($first_category, 'job_listing_category') ) ?>">
                                    <span class="cat-icon" style="background-color: <?php echo esc_attr( get_field('color', 'job_listing_category_' . $first_category->term_id) ? : c27()->defaults()['category']['color'] ) ?>;">
                                        <i class="<?php echo esc_attr( get_field('icon', 'job_listing_category_' . $first_category->term_id) ? : c27()->defaults()['category']['icon'] ) ?>"
                                            style="color: <?php echo esc_attr( get_field('text_color', 'job_listing_category_' . $first_category->term_id) ? : c27()->defaults()['category']['text_color'] ) ?>"></i>
                                    </span>
                                    <span class="category-name"><?php echo esc_html( $first_category->name ) ?></span>
                                </a>
                            </li>
